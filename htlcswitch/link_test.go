...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	ferr, ok := err.(*PaymentError)
	if !ok {
		t.Fatalf("expected a PaymentError, instead got: %T", err)
	}

	switch ferr.GetWireMessage().(type) {
...
func TestUpdateForwardingPolicy(t *testing.T) {
	...
	ferr, ok := err.(*PaymentError)
	if !ok {
		t.Fatalf("expected a PaymentError, instead got (%T): %v", err, err)
	}
	switch ferr.GetWireMessage().(type) {
	case *lnwire.FailFeeInsufficient:
	default:
	...
	ferr, ok = err.(*PaymentError)
	if !ok {
		t.Fatalf("expected a PaymentError, instead got (%T): %v",
			err, err)
	}
	switch ferr.GetWireMessage().(type) {
	case *lnwire.FailTemporaryChannelFailure:
...
func TestChannelLinkMultiHopUnknownNextHop(t *testing.T) {
	...
	fErr, ok := err.(*PaymentError)
	if !ok {
		t.Fatalf("expected PaymentError")
	}
	if _, ok = fErr.GetWireMessage().(*lnwire.FailUnknownNextPeer); !ok {
...
func TestCheckHtlcForward(t *testing.T) {
	...
	t.Run("below minhtlc", func(t *testing.T) {
		result := link.CheckHtlcForward(hash, 100, 50,
			200, 150, 0)
		if _, ok := result.GetWireMessage().(*lnwire.FailAmountBelowMinimum); !ok {
	...
	t.Run("above maxhtlc", func(t *testing.T) {
		result := link.CheckHtlcForward(hash, 1500, 1200,
			200, 150, 0)
		if _, ok := result.GetWireMessage().(*lnwire.FailTemporaryChannelFailure); !ok {
	...
	t.Run("insufficient fee", func(t *testing.T) {
		result := link.CheckHtlcForward(hash, 1005, 1000,
			200, 150, 0)
		if _, ok := result.GetWireMessage().(*lnwire.FailFeeInsufficient); !ok {
	...
	t.Run("expiry too soon", func(t *testing.T) {
		result := link.CheckHtlcForward(hash, 1500, 1000,
			200, 150, 190)
		if _, ok := result.GetWireMessage().(*lnwire.FailExpiryTooSoon); !ok {
	...
	t.Run("incorrect cltv expiry", func(t *testing.T) {
		result := link.CheckHtlcForward(hash, 1500, 1000,
			200, 190, 0)
		if _, ok := result.GetWireMessage().(*lnwire.FailIncorrectCltvExpiry); !ok {
	...
	t.Run("cltv expiry too far in the future", func(t *testing.T) {
		...
		result := link.CheckHtlcForward(hash, 1500, 1000,
			10200, 10100, 0)
		if _, ok := result.GetWireMessage().(*lnwire.FailExpiryTooFar); !ok {
...
func TestChannelLinkCanceledInvoice(t *testing.T) {
	...
	fErr, ok := err.(*PaymentError)
	if !ok {
		t.Fatalf("expected PaymentError, but got %v", err)
	}
	_, ok = fErr.GetWireMessage().(*lnwire.FailIncorrectDetails)
	if !ok {
...
// assertFailureCode asserts that an error is of type PaymentError and that
// the failure code is as expected.
func assertFailureCode(t *testing.T, err error, code lnwire.FailCode) {
	fErr, ok := err.(*PaymentError)
	if !ok {
		t.Fatalf("expected PaymentError but got %T", err)
	}

	if fErr.GetWireMessage().Code() != code {
		t.Fatalf("expected %v but got %v",
			code, fErr.GetWireMessage().Code())
	}
}
