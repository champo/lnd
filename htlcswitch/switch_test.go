...
func TestSkipIneligibleLinksMultiHopForward(t *testing.T) {
	tests := []multiHopFwdTest{
		...
		// available.
		{
			name:          "policy fail",
			eligible1:     true,
			failure1:      NewWireError(lnwire.NewFinalIncorrectCltvExpiry(0)),
		...
		// other channel's policy isn't satisfied.
		{
			...
			eligible1: true,
			failure1: newDetailError(
				lnwire.NewTemporaryChannelFailure(nil),
				&FailureDetailInsufficientBalance{},
			),
			eligible2:     true,
			failure2:      NewWireError(lnwire.NewFinalIncorrectCltvExpiry(0)),
...
func testSkipLinkLocalForward(t *testing.T, eligible bool,
	...
	aliceChannelLink.checkHtlcTransitResult = NewWireError(policyResult)
	if err := s.AddLink(aliceChannelLink); err != nil {
		t.Fatalf("unable to add alice link: %v", err)
	}
...
func TestInvalidFailure(t *testing.T) {
	...
	select {
	case result := <-resultChan:
		fErr, ok := result.Error.(*PaymentError)
		if !ok {
