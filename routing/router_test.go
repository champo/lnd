	...
	ctx.router.cfg.Payer.(*mockPaymentAttemptDispatcher).setPaymentResult(
		func(firstHop lnwire.ShortChannelID) ([32]byte, error) {
			...
			if firstHop == roasbeefSongoku {
				return [32]byte{}, &htlcswitch.PaymentError{
					FailureSourceIdx: 1,
...
// properly gets pruned in response to PaymentError response from the
	...
	for _, test := range tests {
		...
		for _, step := range test.steps {
			...
			case sendToSwitchResultFailure:
				select {
				case sendResult <- &htlcswitch.PaymentError{
					FailureSourceIdx: 1,
					FailureMessage:   &lnwire.FailTemporaryChannelFailure{},
				}:
				case <-time.After(1 * time.Second):
...
func TestSendToRouteStructuredError(t *testing.T) {
	...
	fErr, ok := err.(*htlcswitch.PaymentError)
	if !ok {
